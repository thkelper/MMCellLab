...
from .psccnet import NLCDetection, DetectionHead
...
class CAT_Net(nn.Module):
    def __init__(self, extra=None, pretrained=None, dct_pretrained=None, T=20, **kwargs):
        ...
        self.dc_layer0_dil = nn.Sequential(
            nn.Conv2d(in_channels=T+1,
                    #   out_channels=64,
                      out_channels=extra.dc_other_layer.layer0_out,
                      kernel_size=3,
                      stride=1,
                      dilation=8,
                      padding=8),
            nn.BatchNorm2d(
                            # 64,
                            extra.dc_other_layer.layer0_out,
                            momentum=BN_MOMENTUM),
            nn.ReLU(inplace=True)
        )
        self.dc_layer1_tail = nn.Sequential(
            nn.Conv2d(
                    # in_channels=64, 
                    in_channels=extra.dc_other_layer.layer0_out,
                    out_channels=4, 
                    kernel_size=1, 
                    stride=1, 
                    padding=0, 
        ...
        self.dc_layer2 = self._make_layer(BasicBlock, 
                                        inplanes=4 * 64 * 2, 
                                        # planes=96,
                                        planes= self.dc_stage3_cfg.NUM_CHANNELS[0], 
                                        blocks=4, 
                                        stride=1)
        ...
        self.dc_transition2 = self._make_transition_layer(
                                                    [self.dc_stage3_cfg.NUM_CHANNELS[0]], 
                                                    num_channels)
        ...
        self.seg_net = NLCDetection(None, num_channels, reshape=False)
        self.cls_net = DetectionHead(None, num_channels, reshape=False)

        self.init_weights(pretrained, dct_pretrained)
    ...
    def init_weights(self, pretrained_rgb='', pretrained_dct='',):
        ...
        if pretrained_rgb and os.path.isfile(pretrained_rgb):
            loaded_dict = torch.load(pretrained_rgb, map_location='cpu')
            model_dict = self.state_dict()
            loaded_dict = {k: v for k, v in loaded_dict.items()
                               if k in model_dict.keys() and not k.startswith('lost_layer.')}  # RGB weight
            print('=> (RGB) loading pretrained model {} ({})'.format(pretrained_rgb, len(loaded_dict)))
            model_dict.update(loaded_dict)
            ret = self.load_state_dict(model_dict)
            print('=> (RGB) loaded pretrained model {} ({})'.format(pretrained_rgb, len(loaded_dict)), ret)
        else:
            print('=> Cannot load pretrained RGB')
        if pretrained_dct and os.path.isfile(pretrained_dct):
            loaded_dict = torch.load(pretrained_dct, map_location='cpu')
            model_dict = self.state_dict()
            loaded_dict = {k: v for k, v in loaded_dict.items()
                               if k in model_dict.keys()}
            loaded_dict = {k:v for k,v in loaded_dict.items()
                           if not k.startswith('last_layer')}
            print('=> (DCT) loading pretrained model {} ({})'.format(pretrained_dct, len(loaded_dict)))
            model_dict.update(loaded_dict)
            ret = self.load_state_dict(model_dict)
            print('=> (DCT) loaded pretrained model {} ({})'.format(pretrained_dct, len(loaded_dict)), ret)
        else:
            print('=> Cannot load pretrained DCT')


@BASE_MODELS.register_module()
class CAT_Net_ORI(nn.Module):
    def __init__(self, extra=None, pretrained=None, dct_pretrained=None, cls_head_cfg=None, fuse_mode='concat', skip_dct=False, **kwargs):
        super().__init__()

        self.fuse_mode = fuse_mode
        self.skip_dct = skip_dct
        ...
        if not self.skip_dct:
            if self.fuse_mode == 'concat':
                channels = [i+j for (i, j) in zip(RGB_final_channels, DC_final_stage_channels)]
            elif self.fuse_mode in ('add', 'mul'):
                # RGB feature channels equals to DCT‘s
                channels = RGB_final_channels
        else:
            channels = RGB_final_channels

        self.transition4 = self._make_transition_layer(channels, num_channels)
        ...
        self.cls_head = ClassificationHead(last_inp_channels, **cls_head_cfg)
    ...
    def forward(self, x, qtable):
        ...
        if not self.skip_dct:
            ...
            if self.fuse_mode == 'concat':
                ...
                x = [RGB_list[i+1]+DC_list[i] for i in range(self.stage5_cfg['NUM_BRANCHES']-1)]
