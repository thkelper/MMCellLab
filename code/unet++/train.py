...
from utils import AverageMeter, str2bool, read_txt
...
def parse_args():
    ...
    parser.add_argument('--ann_file_path', default="/mnt/d/ycp/data/agarose_stamp/all_info.txt")
    parser.add_argument('--dataset', default='agarose_stamp',
                        help='dataset name')
...
def main():
    ...
    img_mask_list = read_txt(config['ann_file_path'])
    # img_ids = list(range(len(img_mask_list))) 
    train_img_ids, val_img_ids = train_test_split(img_mask_list, test_size=0.2, random_state=41)
    #数据增强：
    train_transform = Compose([
        A.RandomRotate90(),
        A.Flip(),
        OneOf([
            A.transforms.HueSaturationValue(),
            A.transforms.RandomBrightness(),
            A.transforms.RandomContrast(),
        ], p=1),#按照归一化的概率选择执行哪一个
    ...
    train_dataset = DatasetV2(
        ...
        data_root="/mnt/d/ycp/data/agarose_stamp")
