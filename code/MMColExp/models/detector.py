...
class Detector(BaseModule, metaclass=ABCMeta):
    def __init__(self,
                 base_model,
                 train_cfg=None,
                 test_cfg=None,
        ...
        self.edge_loss = None
        if 'edge_loss' in train_cfg:
            self.edge_loss = build_loss(train_cfg.edge_loss)
    ...
    def compute_seg_loss(self, loss_func, features, gt_mask):
        ...
        down_sample_gt_mask = F.interpolate(gt_mask.to(features.dtype), size, mode='bilinear', align_corners=True)
        return loss_func(features, down_sample_gt_mask)

    def compte_clf_loss(self, att_feature, gt_label):
        # gt_label =  gt_mask.flatten(start_dim=2).max(dim=-1)[0]
        # pred = att_feature.flatten(start_dim=2).max(dim=-1)[0]
        pred = att_feature.flatten(start_dim=2).max(dim=-1)[0]

        return self.clf_loss(pred, gt_label)
    ...
    def forward_train(self, img, img_metas, gt_semantic_seg, 
        ...
        if self.edge_loss:
            edge_feature, att_feature = self.base_model(img)
            edge_loss = self.compute_seg_loss(self.edge_loss,
                                              edge_feature, 
                                              edge_gt_semantic_seg if edge_gt_semantic_seg is not None\
                                                            else gt_semantic_seg)
            losses['edge_loss'] = edge_loss
        else:
            att_feature = self.base_model(img)

        # seg_loss = self.seg_loss(att_feature, gt_semantic_seg)
        seg_loss = self.compute_seg_loss(self.seg_loss, att_feature, gt_semantic_seg)
        losses['seg_loss'] = seg_loss

        if self.clf_loss:
            clf_loss = self.compte_clf_loss(att_feature, img_label)
    ...
    def inference(self, img, img_meta, rescale, argmax=True):
    ...
    def simple_test(self, img, img_meta, rescale=True, argmax=True):
        """Simple test with single image."""
        seg_pred = self.inference(img, img_meta, rescale, argmax=argmax)
        seg_pred = seg_pred.cpu().numpy()
        # seg_pred = np.array(transform_pil(seg_pred))
        # unravel batch dim
        # seg_pred = [seg_pred, ]
        return seg_pred
